...
- **Gateway/non-CLI tool-approval routing.** `_make_claude_code_approval_callback` (Task 8) falls back to `prompt_dangerous_approval` (a CLI-only function) when no thread-local `approval_callback` is registered — which is always the case in gateway (Discord/Telegram/Slack) contexts. This still fails closed (denies) rather than insecurely allowing, so it's not a security gap, but it means every tool Claude requests is auto-denied when this runtime is driven from the gateway rather than the CLI. Codex's equivalent (`CodexAppServerSession._decide_exec_approval`/`_decide_apply_patch_approval`) fails closed to a bare `"decline"` directly in this situation rather than attempting a CLI-only prompt function; a follow-up should either match that, or wire in the actual gateway approval-queue path (`tools.approval._run_approval_gate`'s gateway branch, `_await_gateway_decision`). Until then, `claude_code_sdk` mode is CLI-first — flagged explicitly here per the final whole-branch review rather than left as a silent gap.
- **Memory/skill/compaction side effects the Codex runtime fires that this one doesn't.** `run_codex_app_server_turn` bumps `_iters_since_skill`, calls `_sync_external_memory_for_turn`, may `_spawn_background_review`, and feeds `agent.context_compressor.update_from_response(...)` for compaction tracking (`_record_codex_app_server_usage`/`_record_codex_app_server_compaction`). `run_claude_code_sdk_turn`/`_record_claude_code_sdk_usage` do none of these. This may turn out to be fine — Claude's own SDK manages its context/compaction internally, so Hermes-side compression tracking may be redundant for this runtime — but that's an open question this plan doesn't resolve, not a settled decision. Recorded here explicitly (per the final whole-branch review) so a future maintainer sees this as a known, intentional omission to revisit rather than assuming parity with Codex.
